...
namespace TCPSmart.Ws
{
    public partial class WsManager : Form
    {
        ...
        public WsManager(int id)
        {
            ...
            editRow = DBUtil.GetSQL("SELECT Active,Ambiente,Url,UserAtm,PwdAtm,PLogin_Id,Papp_Code,PLevl_Typ,PMerchant,PPasword,Papp_Vers,LicToken,Id FROM WsParams WHERE Id = " + id);
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (!IsEditing && DBUtil.GetSQL("SELECT * FROM WsParams WHERE LicToken ='" + TxtLic.Text + "' AND Active = 1").Rows.Count > 0)
                    {
                        MessageBox.Show("La Licencia para el ambiente debe ser unica", TCPUtil.AppName, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
